def get_params(function_info, args, kwargs):
    ...
    if function_info["defaults"]:
        for arg, default in zip(
            reversed(function_info["args"]), reversed(function_info["defaults"])
        ):
            params[arg] = default
    ...
    args_and_kwargs = {**dict(zip(function_info["args"], args)), **kwargs}
    ...
    params.update(
        {
            ...
            for i, arg in enumerate(
                args[
                    len(function_info["args"]) : len(args)
                    - len(function_info["defaults"])
                ]
            )
        }
    )
